...
import List from "@/pages/listado";
...
describe("render Home", () => {
  ...
  it("should have button to redirect", () => {
    render(<Home />);
    const myButton = screen.getByText("¡Explorar Ahora!");
    expect(myButton).toBeInTheDocument();
  });

  it("should redirect to listado page", async () => {
    render(<Home />);
  ...
  it("should render insert button on listado page", () => {
    ...
    const insertButton = getByText("Insertar");
    expect(insertButton).toBeInTheDocument();
  }, 5000);

  it("should render Filtrar po descripcion input on listado page", () => {
    fetchMock.mockResponseOnce(JSON.stringify({ data: posts }));
    ...
    const serachInput = getByPlaceholderText("Filtrar por descripcion");
    expect(serachInput).toBeInTheDocument();
  });

  it("should delete element from table in listado page", async () => {
    fetchMock.mockResponseOnce(JSON.stringify({ data: posts }));
    const { getByText } = render(
      <PageProvider>
        <ListProvider>
    ...
    await waitFor(() => {
      const rowToDelete = getByText("qui est esse");
      expect(rowToDelete).toBeInTheDocument();
      const deleteButton = rowToDelete.querySelector("delete");
      if (deleteButton) {
  ...
  it("should get number bigger than 101", async () => {
    const numberGenerated = generateRandomNumber();
    expect(numberGenerated).toBeGreaterThan(101);
  });

  it("should open Modal to insert post", async () => {
    fetchMock.mockResponseOnce(JSON.stringify({ data: posts }));
    const { getByText } = render(
      <PageProvider>
        <ListProvider>
    ...
    await waitFor(() => {
      ...
      const modalOpened = getByText("Inserte la informacion de la publicacion")
